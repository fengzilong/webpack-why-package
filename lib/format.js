const chalk = require( 'chalk' )
const { normalizePath, prettyPath } = require( './shared' )

const atReg = /^@/g
module.exports = function format( assets ) {
  let out = ''

  const emptyAssets = []

  assets.forEach( asset => {
    const { packages = [] } = asset

    if ( packages.length > 0 ) {
      out = out + `\n${ chalk.green( asset.name ) }\n\n`

      packages.forEach( package => {
        out = out + `  ${ chalk.blue( package.name ) } ${ chalk.dim( 'is issued by' ) }\n`

        if ( package.externalIssuers.length > 0 ) {
          package.externalIssuers.sort( ( a, b ) => {
            if ( containsNodeModules( a.name ) && !containsNodeModules( b.name ) ) {
              return 1
            }

            if ( !containsNodeModules( a.name ) && containsNodeModules( b.name ) ) {
              return -1
            }

            if ( containsAt( a.name ) && !containsAt( b.name ) ) {
              return -1
            }

            if ( !containsAt( a.name ) && containsAt( b.name ) ) {
              return 1
            }

            return a.name.replace( atReg, '' ) - b.name.replace( atReg, '' )
          } )

          out = out + package.externalIssuers.map( issuer => {
            return `\n    → ${ prettyPath( issuer.name ) }`
          } ).join( '\n' )
        } else {
          out = out + `\n    → ${ chalk.bold( 'Self' ) } ${ chalk.dim( '( As Entrypoint )' ) }`
        }

        out = out + '\n\n'
      } )
    } else {
      emptyAssets.push( asset )
    }
  } )

  if ( emptyAssets.length > 0 ) {
    out = out + emptyAssets.map( asset => `${ chalk.green( asset.name ) }` ).join( ',\n' )
    out = out + '\n\n  No package found for these assets\n\n'
  }

  return out
}

function containsNodeModules( path ) {
  return path.includes( 'node_modules' )
}

function containsAt( path ) {
  return path.includes( '@' )
}